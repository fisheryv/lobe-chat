...
- [自定义插件流程](#自定义插件流程)
  ...
  - [**`2`** 在 PlanChat 角色设置中添加本地插件](#2-在-lobechat-角色设置中添加本地插件)
...
本节将会介绍如何在 PlanChat 中添加和使用一个自定义插件。
...
### **`2`** 在 PlanChat 角色设置中添加本地插件

接下来进入到 PlanChat 中，创建一个新的助手，并进入它的会话设置页：
...
当完成这些操作后，你已经了解了添加自定义插件，并在 PlanChat 中使用的基础流程。
...
3. `ui`：这个字段包含了插件的用户界面信息，指明了 PlanChat 从哪个地址加载插件的前端界面。由于 PlanChat 插件界面加载是基于 iframe 实现的，因此可以按需指定插件界面的高度、宽度。
4. `gateway`：这个字段指定了 PlanChat 查询 api 接口的网关。PlanChat 默认的插件网关是云端服务，而自定义插件的请求需要发给本地启动的服务，远端调用本地地址，一般调用不通。gateway 字段解决了该问题。通过在 manifest 中指定 gateway，PlanChat 将会向该地址发送插件请求，本地的网关地址将会调度请求到本地的插件服务。发布到线上的插件可以不用指定该字段。
...
由于 PlanChat 默认的插件网关是云端服务 `/api/plugins`，云端服务通过 manifest 上的 `api.url` 地址发送请求，以解决跨域问题。

针对自定义插件，插件请求需要发送给本地服务， 因此通过在 manifest 中指定网关 (<http://localhost:3400/api/gateway>)，PlanChat 将会直接请求该地址，然后只需要在该地址下创建对应的网关即可。
...
[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) 包含了 PlanChat 中插件网关的[实现](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts)，你可以直接使用该包创建网关，进而让 PlanChat 访问到本地的插件服务。
...
PlanChat 通过 `iframe` 实现插件 ui 的加载，使用 `postMessage` 实现主体与插件的通信。因此， 插件 UI 的实现方式与普通的网页开发一致，你可以使用任何你熟悉的前端框架与开发语言。
...
其中关于插件通信，我们在 [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) 提供了相关方法，用于简化插件与 PlanChat 的通信。你可以通过 `fetchPluginMessage` 方法主动向 PlanChat 获取当前消息的数据。关于该方法的详细介绍，参见：[fetchPluginMessage][fetch-plugin-message-url]。
