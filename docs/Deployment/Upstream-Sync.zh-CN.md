...
Docker 部署版本的升级非常简单，只需要重新部署 PlanChat 的最新镜像即可。 以下是执行这些步骤所需的指令：

1. 停止并删除当前运行的 PlanChat 容器（假设 PlanChat 容器的名称是 `lobe-chat`）：
...
2. 拉取 PlanChat 的最新 Docker 镜像：

```fish
docker pull lobehub/lobe-chat
```

3. 使用新拉取的镜像重新部署 PlanChat 容器：
...
> 重新部署的话，我本地的聊天记录会丢失吗？
>
> 放心，PlanChat 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 PlanChat 时，你的聊天记录并不会丢失。
